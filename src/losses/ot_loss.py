    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                # z = im_points[:, 2].unsqueeze_(1)
                x_dis = -2 * torch.matmul(x, self.cood) + x * x + self.cood * self.cood # [#gt, #cood]
                y_dis = -2 * torch.matmul(y, self.cood) + y * y + self.cood * self.cood
                # z_dis = -2 * torch.matmul(z, self.cood) + z * z + self.cood * self.cood
                y_dis.unsqueeze_(2)
                x_dis.unsqueeze_(1)
                # z_dis1 = torch.unsqueeze(z_dis, 1)
                # z_dis2 = torch.unsqueeze(z_dis, 2)
                
                dis = y_dis + x_dis # + z_dis1 + z_dis2
